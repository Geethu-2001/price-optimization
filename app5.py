...
# Load dataset
data_path = os.path.join(os.path.dirname(__file__), "lemonade_price_optimization.csv")
...
st.write("This app predicts the optimal price using different models based on selected features.")

# User Feature Selection
st.subheader("Select Features for Each Model")
selected_features_xgb = st.multiselect("Select Features for XGBoost Model:", X.columns.tolist(), default=X.columns.tolist())
selected_features_q = st.multiselect("Select Features for Q-Learning Model:", X.columns.tolist(), default=X.columns.tolist())
selected_features_bayesian = st.multiselect("Select Features for Bayesian Model:", X.columns.tolist(), default=X.columns.tolist())
selected_features_game_theory = st.multiselect("Select Features for Game Theory Model:", X.columns.tolist(), default=X.columns.tolist())

X_train_xgb, X_test_xgb = X_train[selected_features_xgb], X_test[selected_features_xgb]
X_train_q, X_test_q = X_train[selected_features_q], X_test[selected_features_q]
X_train_bayesian, X_test_bayesian = X_train[selected_features_bayesian], X_test[selected_features_bayesian]
X_train_game_theory, X_test_game_theory = X_train[selected_features_game_theory], X_test[selected_features_game_theory]
...
q_table = np.zeros((len(X_train_q), len(actions)))

def q_learning_price():
    state_idx = np.random.randint(0, len(X_train_q))  # Random state
